...
export class AdvancedCrawler {
  ...
  async crawlUrl(url: string): Promise<CrawledProperty | null> {
    try {
  ...
  private async extractPropertyData(html: string, url: string): Promise<CrawledProperty | null> {
  ...
  private extractStructuredData($: cheerio.CheerioAPI): StructuredData | null {
    try {
  ...
  private extractOpenGraphData($: cheerio.CheerioAPI): OpenGraphData {
  ...
  private extractHTMLData($: cheerio.CheerioAPI): any {
    const data: any = {}
  ...
  private extractBedrooms(text: string): number | null {
  ...
  private extractBathrooms(text: string): number | null {
  ...
  private extractParking(text: string): number | null {
  ...
  private determinePropertyType(title: string, description: string, structuredData: StructuredData | null): string {
  ...
  private extractFeatures(title: string, description: string, structuredData: StructuredData | null): string[] {
  ...
  private extractFinancing(description: string): string | undefined {
  ...
  private extractConditions(description: string): string | undefined {
    const text = description.toLowerCase()
  ...
  async saveProperty(property: CrawledProperty): Promise<string> {
